...
export async function bookToken({ uid, service, totalPrice }) {
  ...
  try {
    ...
    if (userDocSnap.exists()) {
      ...
      fetchedName = userData.displayName || userData.name || auth.currentUser?.displayName || "Customer";
      fetchedPhotoURL = userData.photoURL || auth.currentUser?.photoURL || "";
    } else if (auth.currentUser) {
      fetchedName = auth.currentUser.displayName || "Customer";
      fetchedPhotoURL = auth.currentUser.photoURL || "";
    }
  } catch (err) {
...
export const setStatus = updateTokenStatus;

// Token එක Re-insert කිරීම (Skipped token එකක් නැවත waiting queue එකට දමයි)
export async function skipAndReinsert(tokenId) {
  const tokenRef = doc(db, "tokens", tokenId);
  await updateDoc(tokenRef, {
    status: "waiting",
    order: Date.now(),
